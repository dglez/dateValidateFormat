...
FLAGS =	-Wall -w
LIBS = -lm
READ_OBJECT = readDate.o
FORMAT_OBJECT = format.o
GENERAL_OBJECT = functions.o
HEADER = general.h
OBJS = functions.o readDate.o format.o

all: $(GENERAL_OBJECT) read format clean

# Link file and format file.
read:	$(READ_OBJECT)
		$(COMPILER) $(FLAGS) $(READ_OBJECT) $(GENERAL_OBJECT) \
		-o validateDate $(LIBS)

format:	$(FORMAT_OBJECT)
		$(COMPILER) $(FLAGS) $(FORMAT_OBJECT) $(GENERAL_OBJECT) \
		-o format $(LIBS)

#Compile each individual objects with headers
$(OBJS) : $(HEADER)
